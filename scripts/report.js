// HTML report renderer for ollama-security. Self-contained, no deps.
const esc = (s) => String(s ?? "").replace(/[&<>"]/g, (c) => ({ "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;" }[c]));

const SEV_COLOR = { critical: "#dc2626", high: "#f97316", medium: "#eab308", low: "#3b82f6", info: "#6b7280" };

export function renderHtml(result) {
  const { ollama_url, summary, findings, active_probe, ollama_version } = result;
...
<title>ollama-security report — ${esc(ollama_url)}</title>
<style>body{font-family:system-ui,-apple-system,sans-serif;background:#f3f4f6;margin:0;color:#111827}.wrap{max-width:760px;margin:0 auto;padding:24px}.card{background:#fff;padding:18px;border-radius:8px;box-shadow:0 1px 2px rgba(0,0,0,.06)}</style></head>
<body><div class="wrap"><h1>🛡️ Ollama Security Report</h1>
<div class="card" style="background:#ecfdf5;border:1px solid #a7f3d0">✅ ${esc(result.note || "Not anonymously reachable — good.")}</div></div></body></html>`;
  }
  ...
  const rows = findings.map((f) => `
    <div style="border-left:4px solid ${SEV_COLOR[f.severity]};background:#fff;padding:14px 18px;border-radius:6px;margin-bottom:12px;box-shadow:0 1px 2px rgba(0,0,0,.06)">
      ...
      ${f.probe?.confirmed ? `<div style="font-size:12px;color:#b91c1c;margin-top:4px">✓ CONFIRMED via anonymous probe${f.details?.model_count != null ? ` — ${f.details.model_count} models reachable` : ""}</div>` : ""}
...
<body><div class="wrap">
  <h1 style="margin:8px 0">🛡️ Ollama Security Report</h1>
  <div style="color:#6b7280;font-size:14px">${esc(ollama_url)}${ollama_version ? ` · v${esc(ollama_version)}` : ""} · scored ${score}/100 · ${active_probe.confirmed} issue(s) confirmed live</div>
  <div class="card" style="margin-top:14px">
    ...
    <span class="stat"><div class="n">${active_probe.probed}</div><div class="l">PROBES</div></span>
  </div>
  ...
  <div class="card" style="background:#ecfdf5;border:1px solid #a7f3d0">
    ...
    <a href="https://github.com/Perufitlife/ollama-security" style="color:#047857;font-size:14px;margin-left:10px">See the tool</a>
  </div>` : ""}
  <div style="text-align:center;font-size:12px;color:#9ca3af;padding:8px">
    Generated by <a href="https://github.com/Perufitlife/ollama-security" style="color:#047857">ollama-security</a> · MIT · Runs locally, nothing leaves your machine.
  </div>
</div></body></html>`;
}
